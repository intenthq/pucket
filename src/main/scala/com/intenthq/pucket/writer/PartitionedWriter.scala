...
import com.intenthq.pucket.thrift.ThriftWriter
...
case class PartitionedWriter[T] private (pucket: Pucket[T],
                                         writers: Writers[T, Throwable]) extends Writer[T, Throwable] with
  ...
  override def newInstance(writers: Writers[T, Throwable]): PartitionedWriter[T] =
    PartitionedWriter(pucket, writers)
...
object PartitionedWriter {
  def apply[T](pucket: Pucket[T]): PartitionedWriter[T] =
    PartitionedWriter(pucket, Writers[T, Throwable](Map[String, ThriftWriter[T]](), Map[Long, String]()))
}
